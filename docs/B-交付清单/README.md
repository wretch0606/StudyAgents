...
| 文件 | 行数 | 功能 | 测试 |
|---|---|---|---|
| `parsers/pdf.py` | 479 | PyMuPDF 数字文本提取+页图渲染+扫描件OCR+公式/表格/标题检�?| 18 |
| `parsers/ocr.py` | 261 | OCR 抽象接口+PaddleOCR适配�?Mock降级 | �?|
| `parsers/docx.py` | 10 | 骨架（有余力补） | �?|
| `parsers/pptx.py` | 10 | 骨架 | �?|
| `parsers/image.py` | 10 | 骨架 | �?|
| `validator.py` | 113 | 扩展名→MIME→SHA-256→去�?| 22 |
| `structurer.py` | 133 | 标题层级+公式绑定+表格标注 | 12 |
| `exam_extractor.py` | 195 | 题号检�?11模式)+题型判断(4�?+选项/答案/分�?子题提取 | 18 |
| `chunker.py` | 338 | 500字目标切�?80字重�?短块合并+真题保护 | 20 |
| `vectorizer.py` | 113 | Embedding API+降级确定性伪向量+内容缓存 | 9 |
| `keyword_indexer.py` | 52 | jieba分词+内存倒排索引 | �?|
| `pipeline.py` | 157 | 8阶段全串(校验→解析→OCR→结构化→切块→向量化→索引→完�? | 12 |
