...
| 你负责的模块 | 重点看这几个文件 | 你要确认什�?|
|-------------|-----------------|-------------|
| **B**（知识库/RAG�?| `source-ref.schema.json` | SourceRef 字段是否�?`c/schemas.py` 一致？excerpt 300 字、question_no 可空、page_image_url 格式对吗�?|
| **D**（后�?部署�?| `agent-state.schema.json`、`error.schema.json`、`agent-event.schema.json` | AgentState 公私字段能否�?API 响应中剥离？17 个错误码能否映射？SSE 事件格式能否生成�?|
...
| 文件 | 方向 | 用�?|
|------|------|------|
| `source-ref.schema.json` | B �?C | 检索返回的可引用证据，含文档名、页码、题号、摘录、页�?URL |
| `agent-event.schema.json` | C �?D �?E (SSE) | Agent 步骤事件，前端抽屉展示�? 种事件类型，5 种状�?|
...
| 文件 | 场景 | 用�?|
|------|------|------|
| `mock/qa-success.json` | 问答成功 | �?2 �?SourceRef + 4 个事�?|
| `mock/qa-refusal.json` | 证据不足拒答 | topic_mismatch，含拒答模板 |
| `mock/training-question.json` | 训练题目（提交前�?| **隐私检查关键场�?*——不含答�?rubric |
...
校验内容：Schema 元信息完整�?�?Mock 加载 �?隐私边界扫描 �?字段类型校验
...
- `step_scores`（完整评分点�?
- 任何�?`visibility=staff_only` 的检索块

## 变更流程

1. C 修改 Schema �?更新对应 Mock �?重新运行 `validate.py`
2. 涉及跨模块字段时先群�?@ 对应成员
3. PR 合并后其他人才能基于�?Schema 开�?
